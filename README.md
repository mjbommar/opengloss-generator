...
| Reconcile relations | `relation-reconcile` | Free: apply the stricter of two disagreeing directional verdicts on a symmetric pair, take every demoted `see_also` out of the sense's relation list (recorded to provenance, so nothing is lost) and cap each sense's per-type runs. This is what shortens the list the QA judge is shown; reciprocity goes up, not down. Idempotent; `--dry-run` computes every edit and writes nothing (D-65). |
