...
from opengloss_generator.workflows.queries import (
    DEFAULT_PER_SENSE as QUERIES_DEFAULT_PER_SENSE,
)
from opengloss_generator.workflows.queries import (
    SenseReport,
    plan_queries,
...
#: Per-call token estimates for ``queries --dry-run``. Measured, not modelled, on D-55's
#: luna pilot — the default model — over 60 real entries of ``data/sample-300``
#: (2026-09-03, 218 calls, 12 queries per sense): mean 2,268 input tokens of which 1,999
#: were served from the provider's cache, for a mean 330 output tokens. The input barely
#: moves with the entry because it is dominated by the ~1.9K-token static instruction
#: block; the output is the twelve queries plus the model's own reasoning tokens, which
#: are billed as output and are the larger half of it. The nano pilot measured the same
#: prompt (2,250 input, 1,742 cached) for 544 output tokens, which is most of why it cost
#: more. The summary still labels the result an estimate: a sense with eight siblings
#: carries a longer prompt than one with none.
