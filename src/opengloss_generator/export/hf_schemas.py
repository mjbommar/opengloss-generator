...
#: The release version these repos publish. Part of every repo name.
VERSION = "v2.0"
...
class RepoSpec:
    ...
    @property
    def name(self) -> str:
        """Return the repo name without an owner, e.g. ``opengloss-v2.0-senses``."""
        return f"opengloss-{VERSION}-{self.slug}"

    def repo_id(self, owner: str = DEFAULT_OWNER) -> str:
...
def repo_dir_name(spec: RepoSpec) -> str:
...
def resolve_repos(selector: str) -> list[RepoSpec]:
    """Return the repos ``--repos`` selects.

    Args:
        selector: ``"all"`` or a comma-separated list of slugs (``senses,queries``).
            Full repo names (``opengloss-v2.0-senses``) are accepted too.
    ...
    for token in selector.split(","):
        ...
        if not name:
            continue
        slug = name.removeprefix(f"opengloss-{VERSION}-")
...
def _lexeme_keys() -> tuple[FieldSpec, ...]:
    ...
    return (
        ...
        FieldSpec(
            "tier",
            _STR,
            "`core` (frequency ranks 1–10K), `tier2` (10K–50K) or `tier3` "
            "(the folded remainder); see the coverage table.",
        ),
    )
...
def _sense_keys() -> tuple[FieldSpec, ...]:
    ...
    return (
        ...
        FieldSpec("tier", _STR, "`core`, `tier2` or `tier3`."),
    )
...
_LEXICON = RepoSpec(
    ...
    blurb=(
        ...
        "relation edges. Sense-level content lives in `opengloss-v2.0-senses`, joined on "
        "`lexeme_id`."
    ),
...
_SENSES = RepoSpec(
    ...
    blurb=(
        ...
        "`opengloss-v2.0-lexicon` on `lexeme_id` for entry-level content."
    ),
    task_categories=(
        "text-generation",
        "question-answering",
...
senses = load_dataset("mjbommar/opengloss-v2.0-senses", split="train")
...
_DEFINITIONS = RepoSpec(
    ...
    blurb=(
        ...
        "`opengloss-v2.0-senses` on `sense_id`."
    ),
...
_ENCYCLOPEDIA = RepoSpec(
    ...
    blurb=(
        ...
        "entry (D-71). Etymology has its own repo, `opengloss-v2.0-etymology`."
    ),
...
enc = load_dataset("mjbommar/opengloss-v2.0-encyclopedia", "encyclopedia", split="train")
...
_QUERIES = RepoSpec(
    ...
    blurb=(
        ...
        "of the release; the graded relevance judgements live in `opengloss-v2.0-qrels`."
    ),
...
queries = pl.read_parquet("hf://datasets/mjbommar/opengloss-v2.0-queries/data/train-*.parquet")
defs = pl.read_parquet(
    "hf://datasets/mjbommar/opengloss-v2.0-definitions/data/train-*.parquet"
...
_QA_PAIRS = RepoSpec(
    ...
    blurb=(
        ...
        "by joining back to `opengloss-v2.0-definitions`, `-examples` or `-encyclopedia`."
    ),
...
pairs = load_dataset("mjbommar/opengloss-v2.0-retrieval-pairs", split="train")
...
_RETRIEVAL_TRIPLES = RepoSpec(
    ...
    blurb=(
        ...
        "contrast paragraph in `opengloss-v2.0-contrasts` often states in words why the "
...
triples = load_dataset("mjbommar/opengloss-v2.0-retrieval-triples", split="train")
...
path = hf_hub_download(
    "mjbommar/opengloss-v2.0-qrels", "qrels.trec", repo_type="dataset"
)
