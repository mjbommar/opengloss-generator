...
## Iteration 20 — tier 5 judge sample (2026-09-08 13:46)

40 tier-5 entries / 41 senses, seed 7, Opus, $1.82. The first read on WordNet-sourced
entries (canonical gloss = WordNet's definition; everything else generated by the
chain).

| | tier 2 final | tier 3 | tier 4 | **tier 5** |
|---|---|---|---|---|
| mean score | 70.2 | 66.7 | 67.0 | **81.0** |
| entries 80+ / below 60 | 4 / 5 | 2 / 10 | 4 / 11 | **27 / 1** |
| gloss_accurate defect | 12% | 12% | 29% | **7%** |
| distinct_from_other_senses | 10% | 20% | 36% | **0%** |
| examples_fit_sense | 38% | 39% | 38% | **15%** |
| examples_natural | 34% | 48% | 39% | 32% |
| relations_valid | 60% | 66% | 65% | **24%** |
| domain_fits | 21% | 14% | 12% | 10% |

**Read.** The best-scoring tier by eleven points, and the reason is the input, not
the chain: WordNet's sense inventory is curated (one sense per entry here, hence 0%
distinctness defects), its definitions are accurate (7%), and its typed relations
are hand-built (24% invalid, against 60–66% on the LLM-generated graphs). The chain's
own contributions — examples, renditions, domains — land where they do on other
tiers. This is direct evidence for the paper's methods section: the LLM pipeline's
